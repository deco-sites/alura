            ...
            <div class="z-10 flex flex-wrap justify-center gap-[.5rem]">
              {categories.map((item) => (
                <div class="categories__wrapper--home lg:w-full lg:max-w-[200px] min-h-[200px] justify-start">
                  <div
                    style="clip-path: polygon(0 0,100% 0,100% calc(100% - 25px),calc(100% - 25px) 100%,0 100%);"
                    class="bg-[#021017] min-h-[100%] flex flex-col justify-center p-4"
                  >
                    <a
                      href={item.url}
                      class="cursor-pointer absolute top-0 bottom-0 left-0 right-0 h-full w-full pl-0"
                    >
                    </a>
                    <div class="">
                      ...
                      <div style={{ color: item.textColor }}>
                        <span class="inline-block text-[12px]">
                          {item.title}
                        </span>
                        <h2 class="text-[15px] uppercase font-bold leading-[1.5] mt-[5px]">
                          {item.subTitle}
                        </h2>
                      </div>
                    </div>
                    <nav class="block z-[12] text-[12px] leading-[1.5] mt-3">
                      {item.content.map((txts) => (
                        <a
                          href={txts.textUrl}
                          class="no-underline text-[#D7F9FF] categories__calls__description"
                        >
                          {txts.textContent}
                        </a>
                      ))}
